...
describe("runSync", () => {
	...
	describe("edge cases (ticket 11)", () => {
		...
		it("moves the note when the tag keeps its name but the settings re-target it to another folder", async () => {
			const entry = documentEntry({ tags: [{ name: "sync", timestamp: 0 }] });
			const content = documentContent({ cPages: cPages(["page-a"]) });
			const api = fakeApi({
				rootHash: "root-a",
			...
			const first = await runSync(deps, EMPTY_SYNC_INDEX);
			expect(first.index.rows[notebookSyncKey("doc-1", "sync")].notePath).toBe("Old/Notebook.md");

			// Nothing at all changed on the device -- same root hash, same entry hash. Only the mapping
			// in settings now points "sync" somewhere else.
			const second = await runSync({ ...deps, tagRouter: new TagRouter({ sync: "New" }) }, first.index);

			expect(second.index.rows[notebookSyncKey("doc-1", "sync")].notePath).toBe("New/Notebook.md");
			expect(deps.noteStore.move).toHaveBeenCalledWith("Old/Notebook.md", "New/Notebook.md");
			expect(await deps.noteStore.read("Old/Notebook.md")).toBeNull();
			expect(await deps.noteStore.read("New/Notebook.md")).toContain("[!info]- Generated by Tagged Sync");
		});

		it("does not read a trailing slash dropped from the mapping as a re-target (#101)", async () => {
			// "Tagged/" and "Tagged" are the same folder. The mapping is stored as the user typed it,
			// so trimming the slash changes the fingerprint and forces a full scan -- but no row may
			// come out of that scan re-targeted, or the cosmetic edit re-fetches and re-transcribes
			// every note under the tag.
			const entry = documentEntry({ tags: [{ name: "sync", timestamp: 0 }] });
			const content = documentContent({ cPages: cPages(["page-a"]) });
			const api = fakeApi({
				rootHash: "root-a",
			...
			const deps = baseDeps(api, { sync: "Tagged/" });
			const first = await runSync(deps, EMPTY_SYNC_INDEX);
			expect(first.index.rows[notebookSyncKey("doc-1", "sync")].notePath).toBe("Tagged/Notebook.md");

			deps.noteStore.write.mockClear();
			deps.noteStore.move.mockClear();
			api.getContent.mockClear();
			const second = await runSync({ ...deps, tagRouter: new TagRouter({ sync: "Tagged" }) }, first.index);

			expect(second.notesWritten).toBe(0);
			expect(deps.noteStore.write).not.toHaveBeenCalled();
			expect(deps.noteStore.move).not.toHaveBeenCalled();
			expect(api.getContent).not.toHaveBeenCalledWith("doc-1");
		});

		it("writes into the new folder when the note was deleted before the mapping was re-targeted", async () => {
			const entry = documentEntry({ hash: "hash-1", tags: [{ name: "sync", timestamp: 0 }] });
			const content = documentContent({ cPages: cPages(["page-a"]) });
			const api = fakeApi({
				rootHash: "root-1",
			...
			const previousIndex: SyncIndex = {
				rootHash: "root-1",
				mappings: mappingFingerprint({ sync: "Old" }),
				rows: {
					[notebookSyncKey("doc-1", "sync")]: {
						syncKey: notebookSyncKey("doc-1", "sync"),
						docId: "doc-1",
			...
			const deps = baseDeps(api, { sync: "New" });
			// Note deliberately NOT seeded into the note store: the user deleted it, then re-targeted the tag.

			const result = await runSync(deps, previousIndex);

			expect(deps.noteStore.write).toHaveBeenCalledWith("New/Notebook.md", expect.any(String));
			expect(deps.noteStore.write).not.toHaveBeenCalledWith("Old/Notebook.md", expect.any(String));
		...
		it("moves a page-level note when its page tag is re-targeted, even though the page itself is unchanged", async () => {
			...
			const deps = baseDeps(api, { todo: "Old" });
			const first = await runSync(deps, EMPTY_SYNC_INDEX);
			expect(first.index.rows[pageSyncKey("doc-1", "page-a", "todo")].notePath).toBe("Old/Notebook — Page 1.md");

			const second = await runSync({ ...deps, tagRouter: new TagRouter({ todo: "New" }) }, first.index);
